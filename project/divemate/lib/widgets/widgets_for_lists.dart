...
Widget customListViewDives(
    List<Dive> _divelist, User user, BuildContext context) {
  /**
    @params
  ...
  _divelist
      .sort((Dive a, Dive b) => a.startDatetime.compareTo(b.startDatetime));
  ...
  final separatorFont = TextStyle(
      fontWeight: FontWeight.bold, color: Colors.blue.shade900.withAlpha(190));
  ...
  return ListView.separated(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        if (_divelist[index]?.img == null ||
            _divelist[index]?.img == "" ||
            _divelist[index]?.img == "LINK_TO_IMG") {
        ...
        var listTile = ListTile(
          ...
          leading: previewIcon,
          title: Text(
            _divelist[index].location,
          ),
          subtitle: Text(
            _divelist[index].comment,
          ),
          trailing: IconButton(
          ...
          onTap: () {
            Navigator.pushNamed(context, SingleDiveScreen.id,
                arguments: {'dive': _divelist[index]});
          },
        );
        ...
        if (index == 0) {
          var seprator = Column(
            children: [
              Text(
                months[_divelist[index].startDatetime.month] +
                    " " +
                    _divelist[index].startDatetime.day.toString() +
                    ", " +
                    _divelist[index].startDatetime.year.toString(),
                style: separatorFont,
              ),
              listTile,
            ],
        ...
        if (!isSameDate(index + 1)) {
          return Column(
            children: [
              Divider(),
              Text(
                months[_divelist[index + 1].startDatetime.month] +
                    " " +
                    _divelist[index + 1].startDatetime.day.toString() +
                    ", " +
                    _divelist[index + 1].startDatetime.year.toString(),
                style: separatorFont,
              ),
            ],
          ); //Divider(),
...
Widget customListViewDocuments(
    List<Document> _doclist, User user, BuildContext context) {
  /**
    @params
  ...
  return ListView.separated(
    ...
    itemBuilder: (BuildContext context, int index) {
      ...
      if (_doclist[index]?.img == null ||
          _doclist[index]?.img == "" ||
          _doclist[index]?.img == "LINK_TO_IMG") {
        previewIcon = Icon(Icons.image, size: 60);
      } else {
        previewIcon = Image.network(
          _doclist[index].img,
      ...
      return ListTile(
        ...
        leading: previewIcon,
        onTap: () => Navigator.pushNamed(context, DocumentForm.id,
            arguments: {'document': _doclist[index]}),
